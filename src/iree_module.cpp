...
Error IREEModule::capture() {
	...
	iree_vm_instance_t *const instance =
			IREEInstance::borrow_singleton()->borrow_assured_vm_instance();
	ERR_FAIL_NULL_V(instance, ERR_CANT_CREATE);

	// Get hal module.
	iree_vm_module_t *const hal_module =
			IREEInstance::borrow_singleton()->borrow_assured_hal_module();
	...
	IREE_ERR_V_MSG(iree_vm_context_create_with_modules(
