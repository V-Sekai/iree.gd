...
Error IREETensor::capture(typename StorageType<T>::type p_data,
  ...
  for (int i = 0; i < shape_rank; i++)
    shape[i] = p_dimension[i];

  iree_hal_device_t *const device =
      IREEInstance::borrow_singleton()->borrow_assured_hal_device();
  ...
  IREE_ERR_V_MSG(
      iree_hal_buffer_view_allocate_buffer_copy(
          device, iree_hal_device_allocator(device), shape_rank, shape, T,
          IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR, buffer_params,
          iree_make_const_byte_span(
              p_data.ptr(), p_data.size() * StorageType<T>::element_size),
          &buffer_view),
...
IREETensor::IREETensor() : buffer_view(nullptr) {}
...
bool IREETensor::is_null() const { return buffer_view == nullptr; }
...
PackedByteArray IREETensor::get_data() const {
  ...
  ERR_FAIL_COND_V_MSG(
      data.size() != data_size, PackedByteArray(),
      "Fail to allocate memory for data in IREE Tensor for retrieving data.");
  ...
  IREE_ERR_V_MSG(
      iree_hal_device_transfer_d2h(
          device, iree_hal_buffer_view_buffer(buffer_view), 0, data.ptrw(),
          data_size, IREE_HAL_TRANSFER_BUFFER_FLAG_DEFAULT,
